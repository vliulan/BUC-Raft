...
impl<T: Transport, C: PdClient> Store<T, C> {
	...
	if has_writes {
        ...
        if !kv_wb.is_empty() {
            ...
            write_opts.set_sync(true); // wyy change true to false
            self.kv_engine
        ...
        if !raft_wb.is_empty() {
            ...
            let mut write_opts = WriteOptions::new();
            write_opts.set_sync(self.cfg.sync_log || sync_log);	// wyy delete
	    //write_opts.set_sync(false); // wyy add
            self.raft_engine
