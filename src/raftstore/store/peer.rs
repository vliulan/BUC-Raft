...
pub struct NoApply {
    ...
    pub value_len: u64, // fast-raft wyy add
}
...
impl Peer {
	...
	//self.handle_wr();       // wyy add
        ...
        let mut value_len = 0;  // fast-raft wyy add
        let res = match self.get_handle_policy(&req) {
            ...
            Ok(RequestPolicy::ReadIndex) | Ok(RequestPolicy::ReadLocal) => {
                ...
                // direct read
                let mut label = 0;  // wyy add. 0为点查询，1为范围查询
		        if req.requests.len() > 0 {
                    key = req.requests[0].get_get().key.clone();    // kv-raft wyy add
                    if req.requests[0].get_put().key.len() > 0 {    // wyy add
                        label = 1;  // 范围查询
                        //println!("scan");
                    }
                ...
                let mut relaxed_read = 0;  // fast-raft wyy add
                let mut relaxed_read_index = 0;  // fast-raft wyy add
                for no_apply in self.no_applies.iter() {
                    if no_apply.index > self.raft_group.raft.raft_log.committed {   // 不能读没commit的数据
                        ...
                        if label == 0 { // 点查询
                            ...
                            if no_apply.key >= key {
                                //println!("key: {:?}, no_apply.key: {:?}", key, no_apply.key);
                                if no_apply.v
                            }
                        }
            ...
            Ok(RequestPolicy::ProposeNormal) => {
                ...
                if req.requests.len() > 0 {
                    ...
                    value_len = req.requests[0].get_put().value.len(); // fast-raft wyy add
                }
