...
fn test_mvcc_basic() {
    ...
    println!("Prewrite k :{:?} , v : {:?} , prewrite_start_version : {:?}", k, v, prewrite_start_version);  // wyy add
    ...
    println!("Commit key :{:?} , commit_version : {:?}", k, commit_version);    // wyy add
    ...
    assert_eq!(get_resp.value, v);
    println!("Get get_req :{:?} , get_resp : {:?}", get_req, get_resp);    // wyy add
